...
fn test_conv2d_cuda() -> Result<()> {
    ...
    let (batch_size, in_channels, height, width) = (2, 3, 32, 32);

    // Create deterministic input tensor with hardcoded values for reproducibility
    let mut input_data = vec![0.0f32; batch_size * in_channels * height * width];
        ...
        for c in 0..in_channels {
            for h in 0..height {
                for w in 0..width {
                    ...
                    let idx = b * in_channels * height * width + c * height * width + h * width + w;
    ...
    let conv_layer = candle_nn::conv2d(
        in_channels,
        out_channels,
        kernel_size,
        conv_config,
    ...
    let avg_time = end_time.duration_since(start_time).as_millis() as f64 / num_runs as f64;
